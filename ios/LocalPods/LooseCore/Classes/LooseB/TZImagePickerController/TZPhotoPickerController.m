...
@property (nonatomic, strong) GJAlbumPickerTableView *albumTableView;
@property (nonatomic, strong) AtControl *albumButton;
...
- (void)viewDidLoad {
    ...
    if (tzImagePickerVc.navLeftBarButtonSettingBlock) {
        ...
        UIBarButtonItem *backItem = [[UIBarButtonItem alloc] initWithTitle:@"返回" style:UIBarButtonItemStylePlain target:self action:@selector(navLeftBarButtonClick)];
...
- (void)initSubviews {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (tzImagePickerVc.isCustomUI) {
            [self.albumTableView configAlbumData];
        }
    });
}
...
- (void)doneButtonClick {
    ...
    if (tzImagePickerVc.minImagesCount && tzImagePickerVc.selectedModels.count < tzImagePickerVc.minImagesCount) {
        NSString *title = [NSString stringWithFormat:@"请至少选择 %zd 张照片", tzImagePickerVc.minImagesCount];
        ...
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            ...
            TZImageRequestOperation *operation = [[TZImageRequestOperation alloc] initWithAsset:model.asset completion:^(UIImage * _Nonnull photo, NSDictionary * _Nonnull info, BOOL isDegraded) {
                ...
                if (progress < 1 && havenotShowAlert && !alertView) {
                    alertView = [tzImagePickerVc showAlertWithTitle:@"正在从iCloud同步照片"];
                    havenotShowAlert = NO;
                    return;
                }
                if (progress >= 1) {
    ...
    if (tzImagePickerVc.selectedModels.count <= 0 || tzImagePickerVc.onlyReturnAsset) {
        if (tzImagePickerVc.isCustomUI) {
            [tzImagePickerVc hideProgressHUD];
            _doneButton.enabled = tzImagePickerVc.alwaysEnableDoneBtn;
            if (tzImagePickerVc.doneButtonClickNoData) {
                tzImagePickerVc.doneButtonClickNoData();
            }
        }else{
            [self didGetAllPhotos:photos assets:assets infoArr:infoArr];
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (notSelectable && tzImagePickerVc.showPhotoCannotSelectLayer && !model.isSelected) {
        ...
        if (tzImagePickerVc.isCustomUI && model.type == TZAssetModelMediaTypeVideo && (tzImagePickerVc.selectedModels.count > 0 || tzImagePickerVc.hasSelectPhoto)) {
            cell.cannotSelectLayerButton.backgroundColor = tzImagePickerVc.cannotSelectLayerColor;
            cell.cannotSelectLayerButton.hidden = NO;
        }else{
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        if (isSelected) {
            ...
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                ...
                NSString *title = [NSString stringWithFormat:@"你最多只能选择 %zd 张照片", tzImagePickerVc.maxImagesCount];
                [tzImagePickerVc showAlertWithTitle:title];
            }
        }
    };
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            NSInteger minLimt = 5, maxLimt = MAXFLOAT;
            if(tzImagePickerVc.reportVideoDuration && tzImagePickerVc.reportVideoDuration.length>0){
                minLimt = [[[tzImagePickerVc.reportVideoDuration componentsSeparatedByString:@","] firstObject] integerValue];
                maxLimt = [[[tzImagePickerVc.reportVideoDuration componentsSeparatedByString:@","] lastObject] integerValue];
            }
            if (roundf(model.asset.duration) < minLimt) {
                [self push:[NSString stringWithFormat:@"视频长度要求至少%ld秒哦",minLimt]];
                return;
            }
            if (tzImagePickerVc.selectVideoBlock) {
                tzImagePickerVc.selectVideoBlock(model.asset);
            }
        ...
        if (tzImagePickerVc.selectedModels.count > 0) {
            TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:@"选择照片时不能选择 GIF"];
        } else {
...
- (void)checkSelectedModels {
    ...
    for (TZAssetModel *model in selectedModels) {
        if(!tzImagePickerVc.allowPickingVideo && model.type == TZAssetModelMediaTypeVideo){
            continue;
        }
        if(!tzImagePickerVc.allowPickingImage && model.type == TZAssetModelMediaTypePhoto){
            continue;
        }
...
#pragma mark - isCustomUI

- (void)setNavigationItemView{
    _albumButton = [[AtControl alloc]init];
    [self updateAlbumButton:@"相册"];
    self.navigationItem.titleView = [self updateAlbumButton:@"相册"];
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    UIBarButtonItem *cancelItem = [[UIBarButtonItem alloc] initWithTitle:tzImagePickerVc.cancelBtnTitleStr style:UIBarButtonItemStylePlain target:tzImagePickerVc action:@selector(cancelButtonClick)];
    [cancelItem setTintColor:tzImagePickerVc.barItemTextColor];
    self.navigationItem.leftBarButtonItem = cancelItem;
    
    self.navigationItem.rightBarButtonItem = nil;
}
- (AtControl *)updateAlbumButton:(NSString *)name {
    
    if (_albumButton != nil) {
        [_albumButton removeFromSuperview];
        _albumButton = nil;
    }
    _albumButton = [[AtControl alloc]initWithFrame:CGRectMake(0, 0, 100, 40)];
    [_albumButton setTitle:name forState:UIControlStateNormal];
    [_albumButton setTitleColor:[ShowColor current] forState:UIControlStateNormal];
    _albumButton.titleLabel.font = [UIFont underbelly:PFSCTypeMedium substance:16];
    [_albumButton setImage:[UserTextImage imageNamed:@"btn2VE3la_otohp_down"] forState:UIControlStateNormal];
    [_albumButton setImage:[UserTextImage imageNamed:@"btnXw7FH1_otohp_up"] forState:UIControlStateSelected];
    [_albumButton addTarget:self action:@selector(albumButtonClickAction:) forControlEvents:UIControlEventTouchUpInside];
    
    [_albumButton setTitleEdgeInsets:UIEdgeInsetsMake(0, -_albumButton.imageView.bounds.size.width-2.5, 0, _albumButton.imageView.bounds.size.width+2.5)];

    [_albumButton setImageEdgeInsets:UIEdgeInsetsMake(0, _albumButton.titleLabel.bounds.size.width+2.5, 0, -_albumButton.titleLabel.bounds.size.width-2.5)];
    
    return _albumButton;
}
- (void)albumButtonClickAction:(UIButton *)sender{
    sender.selected = !sender.selected;
    if (sender.selected) {
        [self.albumTableView show];
    }else{
        [self.albumTableView hide];
    }
    [self.albumTableView setIsShowBlock:^(BOOL isShow) {
        sender.selected = isShow;
    }];
    
}

-(GJAlbumPickerTableView *)albumTableView{
    if (!_albumTableView) {
        _albumTableView = [[GJAlbumPickerTableView alloc]initWithFrame:CGRectMake(0, statusBarNavBarHeight(), screenWidth(), screenHeight())];
        _albumTableView.currentController = self;
        _albumTableView.isFirstAppear = YES;
        @weakify(self);
        [_albumTableView setAlbumModelBlock:^(TZAlbumModel * _Nonnull model) {
            @strongify(self);
            self->_models = [NSMutableArray arrayWithArray:model.models];
            self->_model = model;
            dispatch_async(dispatch_get_main_queue(), ^{
                self.navigationItem.titleView = [self updateAlbumButton:model.name];
            });
            [self checkSelectedModels];
            [self.collectionView reloadData];
        }];
