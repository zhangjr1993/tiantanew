...
- (void)configCustomNaviBar {
    ...
    [_backButton setImage:[UserTextImage imageNamed:@"navi_back"] forState:UIControlStateNormal];
...
- (void)select:(UIButton *)selectButton refreshCount:(BOOL)refreshCount {
    ...
    if (!selectButton.isSelected) {
        ...
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            NSString *title = [NSString stringWithFormat:@"你最多只能选择 %zd 张照片", _tzImagePickerVc.maxImagesCount];
...
- (void)doneButtonClick {
    ...
    if (_progress > 0 && _progress < 1 && (_selectButton.isSelected || !_tzImagePickerVc.selectedModels.count )) {
        _alertView = [_tzImagePickerVc showAlertWithTitle:@"正在从iCloud同步照片"];
        return;
    }
